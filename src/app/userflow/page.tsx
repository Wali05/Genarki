        ...
        if (user) {
          ...
          if (ideasData && ideasData.length > 0) {
            ...
            if (!selectedIdeaId && mergedIdeas.length > 0) {
              const ideaWithBlueprint = mergedIdeas.find(idea => !!mergedBlueprints[idea.id]?.user_flow);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden">
        <motion.div 
          className="absolute -top-40 -left-40 w-96 h-96 rounded-full bg-green-500/10 blur-3xl dark:bg-green-600/10"
          animate={{ 
            x: [0, 40, 0],
            y: [0, 60, 0],
          }}
        ...
        />
        <motion.div 
          className="absolute top-1/2 -right-40 w-96 h-96 rounded-full bg-indigo-500/10 blur-3xl dark:bg-indigo-600/10"
          animate={{ 
            x: [0, -40, 0],
            y: [0, -30, 0],
          }}
      ...
      <div className="container py-8 max-w-6xl mx-auto">
        <div className="flex items-center mb-8">
          ...
          <motion.h1 
            className="text-3xl font-bold ml-4 bg-gradient-to-r from-green-600 to-indigo-600 bg-clip-text text-transparent"
        ...
        {loading ? (
          <div className="flex items-center justify-center h-96">
            ...
            >
              <div className="w-12 h-12 rounded-full border-t-2 border-l-2 border-green-600 animate-spin"></div>
              <div className="w-8 h-8 rounded-full border-t-2 border-r-2 border-indigo-600 absolute top-6 left-6 animate-spin"></div>
            </motion.div>
          </div>
        ) : ideas.length === 0 ? (
          <motion.div 
          ...
          >
            <div className="rounded-full bg-gradient-to-br from-green-100 to-indigo-100 dark:from-green-900/40 dark:to-indigo-900/40 p-6 mb-6 shadow-inner">
              <Users className="h-10 w-10 text-green-600 dark:text-green-400" />
            </div>
            <h3 className="text-xl font-semibold mb-2">No projects found</h3>
            <p className="text-muted-foreground mb-6 max-w-md">
              Generate your first SaaS idea blueprint to view user flow diagrams
            </p>
            <Button 
              className="bg-gradient-to-r from-green-600 to-indigo-600 hover:from-green-500 hover:to-indigo-500 transition-colors"
          ...
          <>
            ...
            >
              ...
              <select 
                className="w-full max-w-md px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                value={selectedIdeaId || ''}
                onChange={(e) => setSelectedIdeaId(e.target.value)}
              >
                <option value="">Select a project...</option>
                {ideas.map(idea => (
                  <option key={idea.id} value={idea.id} disabled={!blueprints[idea.id]?.user_flow}>
                    {idea.title} {!blueprints[idea.id]?.user_flow && '(No user flow available)'}
                  </option>
                ))}
              </select>
            </motion.div>
            ...
            <AnimatePresence mode="wait">
              {selectedIdeaId && getSelectedBlueprint()?.user_flow ? (
                <motion.div
                  key={selectedIdeaId}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  <Card className="border dark:border-gray-800 shadow-md bg-white dark:bg-gray-900">
                    <CardHeader>
                      <CardTitle className="text-xl">{getSelectedIdea()?.title} - User Flow</CardTitle>
                      <CardDescription>
                        Visual representation of how users interact with the application
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                        <MermaidChart 
                          chart={getSelectedBlueprint()?.user_flow || ''} 
                        />
                      </div>
                    </CardContent>
                  </Card>
                ...
                >
                  <p className="text-muted-foreground mb-4">
                    No user flow diagram available for this project.
                  </p>
                  <Button 
                    variant="outline"
